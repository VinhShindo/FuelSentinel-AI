...
function formatFuelDelta(processed) {
    const delta = processed?.WindowFuelDiff ?? processed?.InstantFuelDiff ?? 0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateDashboard(data) {
        ...
        if(legendEl) {
            legendEl.innerHTML = `
                <div class="legend-row fw-bold border-bottom pb-2 mb-1"><span style="background:var(--color-primary);"></span> Fuel Level</div>
                ...
                <div class="legend-row fw-bold border-bottom pb-2 mt-2 mb-1"><span style="background:var(--color-text-primary); width:10px; height:4px;"></span> Boundary Marker</div>
        ...
        if(rulePanel) {
            rulePanel.innerHTML = `
                ...
                <div class="row g-2 small mb-2">
                    ...
                    <div class="col-6"><span class="text-muted">Duration:</span> <strong>${formatStopDurationMinutes(lastProcessed.StopDuration)}</strong></div>
                    <div class="col-6"><span class="text-muted">Fuel Δ:</span> <strong>${formatFuelDelta(lastProcessed)} L</strong></div>
                    <div class="col-6"><span class="text-muted">Slope:</span> <strong>${(lastProcessed.RegressionSlope ?? 0).toFixed(2)}</strong></div>
                    ...
                    <div class="col-12"><span class="text-muted">Confidence:</span> <span style="color: var(--color-success);">${lastProcessed.Confidence > 0 ? lastProcessed.Confidence.toFixed(2) + '%' : 'N/A'}</span></div>
                </div>
            `;
        }
        ...
        updateEl('kpi-speed', lastRaw.Speed?.toFixed(2) ?? '--');
        updateEl('kpi-slope', (lastProcessed.RegressionSlope ?? 0).toFixed(2));
        updateEl('kpi-confidence', lastProcessed.Confidence > 0 ? lastProcessed.Confidence.toFixed(2) + '%' : 'N/A');
        updateEl('kpi-stop', formatStopDurationMinutes(lastProcessed.StopDuration));
